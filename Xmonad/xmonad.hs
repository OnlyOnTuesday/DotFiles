...
import XMonad.Util.EZConfig -- for keybindings, list of keys on website
...
main = do
     ...
     xmonad $ docks def
       ...
       `additionalKeysP`
       [ ("M-S-<Return>", spawn "st" ) --launch a terminal

         --launch firefox developer edition
       , ("M-S-f", spawn myWebBrowser)

         -- lower the volume using the function5 fn
       , ("<XF86AudioLowerVolume>", spawn myLowerVolume)

         -- raise the volume using the function6 fn
       , ("<XF86AudioRaiseVolume>", spawn myRaiseVolume)

         -- mute/unmute using the function3 fn
       , ("<XF86AudioMute>", spawn myToggleMute)

         -- use the print key to take a screenshot
       , ("M-<Print>", spawn "shutter -s")

         -- print the default key bindings
       , ("M-/", spawn ("echo " ++ show help ++ " | xmessage -file -"))
...
--myKeys = [ ("M-S-<Return>", spawn "xterm -c echo hello") ]


-- Key bindings. Add, modify or remove key bindings here.
-- use xev to find keysyms
